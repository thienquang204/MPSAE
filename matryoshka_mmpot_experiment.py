...
def build_parser() -> argparse.ArgumentParser:
    ...
    train.add_argument("--workers", type=int, default=8, help="data-loading worker processes")
    train.add_argument("--prefetch-factor", type=int, default=2, help="batches prefetched by each DataLoader worker")
...
def validate_args(args: argparse.Namespace) -> None:
    if args.epochs < 0 or args.batch_size < 1 or args.workers < 0 or args.prefetch_factor < 1:
        raise ValueError("epochs must be non-negative; batch-size/prefetch-factor positive; workers non-negative")
...
class PairwiseScaleCost(nn.Module):
    """M3-inspired averaged cross-batch cosine cost from nested prefixes."""

    def __init__(self, nested_dims: Sequence[int], feature_dim: int, pairs: str = "all") -> None:
        super().__init__()
        self.nested_dims = tuple(nested_dims)
        self.feature_dim = feature_dim
        if pairs == "adjacent":
            self.scale_pairs = tuple((index, index + 1) for index in range(len(nested_dims) - 1))
        else:
        ...
        if not self.scale_pairs:
            raise ValueError("the MMPOT proxy requires at least two nested dimensions")
    ...
    def forward(self, features: torch.Tensor) -> torch.Tensor:
        ...
        cost = features32.new_zeros((features32.shape[0], features32.shape[0]))
...
class GreenkhornPartialOT(nn.Module):
    """Stabilized two-marginal partial-OT proxy used by the local plan."""

    def __init__(self, mass: float, eta: float, max_iters: int, tol: float, gradient_mode: str) -> None:
        super().__init__()
    ...
    def _solve(self, cost: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        if cost.ndim != 2 or cost.shape[1] != n:
            raise ValueError("partial OT cost must be square [batch, batch]")
        ...
        for iteration in range(self.max_iters):
            ...
            scaled_sum = scaled.sum().clamp_min(self.eps)
            partition = (weight * scaled_sum + v1.sum() + v2.sum()).clamp_min(self.eps)
            factor = normalizer_mass / partition
            r1 = factor * (weight * scaled.sum(dim=1) + v1)
            r2 = factor * (weight * scaled.sum(dim=0) + v2)
            current_mass = factor * weight * scaled_sum
            errors = torch.stack(
                [
...
class MatryoshkaClassifier(nn.Module):
    def __init__(
        ...
        if method == "mmpot":
            self.cost_builder: Optional[PairwiseScaleCost] = PairwiseScaleCost(
                nested_dims, feature_dim, args.ot_scale_pairs
            )
            self.ot_solver: Optional[GreenkhornPartialOT] = GreenkhornPartialOT(
                args.ot_mass, args.ot_eta, args.ot_iters, args.ot_tol, args.ot_grad
            )
        else:
...
def build_model(
    ...
    if method == "mmpot" and len(dims) < 2:
        raise ValueError("mmpot requires at least two nested dimensions")
...
def append_jsonl(path: Path, record: Mapping[str, Any]) -> None:
    """Upsert an epoch record so checkpoint resumes cannot duplicate history."""
    path.parent.mkdir(parents=True, exist_ok=True)
        ...
        with path.open(encoding="utf-8") as handle:
            for line in handle:
                if line.strip():
                    ...
                    by_epoch[int(existing["epoch"])] = existing
...
def run_linear_probe(
    ...
    for epoch in range(args.probe_epochs):
        ...
        probe_history.append(
            {"epoch": epoch + 1, "loss": total_loss / samples, "learning_rate": epoch_lr}
        )
        scheduler.step()
    ...
    return {
        ...
        "samples": samples,
        "training_history": probe_history,
        "per_dim": per_dim,
    }
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    init_wandb(
        args,
        default_name=(
            f"pairwise-{args.architecture}-{args.method}-{args.optimizer}"
        ),
        default_group="pairwise-mmpot",
        extra_config={
            "experiment_family": "pairwise_mmpot_proxy",
            "image_size": image_size,
            "num_classes": bundle.num_classes,
        },
        tags=("pairwise-mmpot", args.architecture, args.optimizer),
    )
    print(
    ...
    summary["reporting"] = generate_matryoshka_report(
        args.output_dir,
        summary,
        filename_prefix="pairwise_mmpot",
    )
